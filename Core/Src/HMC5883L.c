...
//###############################################################################################################
uint8_t QMC_init(QMC_t *qmc,uint8_t Output_Data_Rate)
{
	uint8_t array[2];
	...
	i2c1_MemoryWrite_Byte(QMC_ADDR, 0x0B, array[0]);
	i2c1_MemoryWrite_Byte(QMC_ADDR, 0x09, array[1]);
...
//	  HAL_I2C_Mem_Read(qmc->i2c, 0x1A, 0x06, 1, qmc->datas, 1, 100);
		i2c1_readMemoryByte(QMC_ADDR, 0x06, qmc->datas);
	  if((qmc->datas[0]&0x01)==1)
	  {
//		  HAL_I2C_Mem_Read(qmc->i2c, 0x1A, 0x00, 1, qmc->datas, 6, 100);
			i2c1_readMemoryMulti(QMC_ADDR, 0x00, qmc->datas, 6);
...
//	if(HAL_I2C_Mem_Write(qmc->i2c, 0x1A, 0x09, 1, &array[0], 1, 100)!=HAL_OK)return 1;
	i2c1_MemoryWrite_Byte(QMC_ADDR, 0x09, array[0]);
	return 0;
}
...
//	if(HAL_I2C_Mem_Write(qmc->i2c, 0x1A, 0x0A, 1, &array[0], 1, 100)!=HAL_OK)return 1;
	i2c1_MemoryWrite_Byte(QMC_ADDR, 0x0A, array[0]);
	return 0;
}